    ...
    <div class="bg-gradient-to-br from-blue-100 via-green-100 to-yellow-100 min-h-screen py-16 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="text-center mb-12">
                <h1 class="text-4xl font-sans text-indigo-800 drop-shadow">Hoi {{ Auth::user()->name }}</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                ...
                <div class="md:col-span-2 bg-white rounded-3xl shadow-2xl p-8 space-y-6">
                    <h2 class="text-xl font-sans text-indigo-700 border-b pb-2">Profiel Bewerken</h2>
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-sans text-gray-700">Naam</label>
                            <input type="text" name="name" value="{{ old('name', Auth::user()->name) }}"
                                   class="w-full p-3 mt-1 border border-indigo-200 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-400" />
                        </div>

                        <div>
                            <label class="block text-sm font-sans text-gray-700">Email</label>
                            <input type="email" name="email" value="{{ old('email', Auth::user()->email) }}"
                                   class="w-full p-3 mt-1 border border-indigo-200 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-400" />
                        </div>

                        <div>
                            <label class="block text-sm font-sans text-gray-700">Avatar</label>
                    ...
                    <form method="POST" action="{{ route('password.update') }}" class="pt-8 border-t space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-sans text-gray-700">Huidig wachtwoord</label>
                            <input type="password" name="current_password"
                                   class="w-full p-3 mt-1 border border-red-200 rounded-xl shadow-sm focus:ring-2 focus:ring-red-400" />
                        </div>

                        <div>
                            <label class="block text-sm font-sans text-gray-700">Nieuw wachtwoord</label>
                            <input type="password" name="password"
                                   class="w-full p-3 mt-1 border border-red-200 rounded-xl shadow-sm focus:ring-2 focus:ring-red-400" />
                        </div>

                        <div>
                            <label class="block text-sm font-sans text-gray-700">Bevestig wachtwoord</label>
                        ...
                        <div class="text-right">
                            <button type="submit"
                                    class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-full font-sans shadow transition">
                                🔐 Wachtwoord wijzigen
                            </button>
                        </div>
                    </form>
                ...
                <div class="bg-white rounded-3xl shadow-xl p-6 text-center">
                    ...
                    <h3 class="text-lg font-sans text-indigo-700">{{ Auth::user()->name }}</h3>
